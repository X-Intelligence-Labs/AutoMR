...
oppo_train = mhealth_dataset(
    ...
    window_size=15,
    stride=7,
    annotation_path='oppo/clean_data/processed/s3y_train.txt'  # 提供注释文件路径
)
...
oppo_test = mhealth_dataset(
    ...
    window_size=15,
    stride=7,
    annotation_path='oppo/clean_data/processed/s3y_test.txt'  # 提供注释文件路径
)
...
def remap_labels(labels):
    """
    Remap discontinuous labels to continuous labels starting from 0.
    Args:
        labels (np.ndarray): Original labels.
    Returns:
        np.ndarray, dict: Remapped labels and mapping dictionary.
    """
