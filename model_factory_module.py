...
class CNNModel(nn.Module):
    ...
    def __init__(self, input_shape, output_shape):
        ...
        self.batch_size, self.height, self.width = input_shape
        ...
        conv_layers = []
        previous_num_filters = 1
        for num_filters in self.filters:
            conv_layers.append(nn.Conv2d(previous_num_filters, num_filters, kernel_size=3, padding=1))
            conv_layers.append(nn.BatchNorm2d(num_filters))
            conv_layers.append(nn.ReLU())
            conv_layers.append(nn.Conv2d(num_filters, num_filters, kernel_size=3, padding=1))
            conv_layers.append(nn.BatchNorm2d(num_filters))
            conv_layers.append(nn.ReLU())
            conv_layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
            conv_layers.append(nn.Dropout2d(p=0.2))
        ...
        self.fc_layers = nn.Sequential(
            nn.Flatten(),
            nn.Linear(128 * (self.height // 16) * (self.width // 16), 512),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.4),
            nn.Linear(512, 256),
    ...
    def forward(self, x):
        x = x.view(-1, 1, self.height, self.width).float()  # Reshape input
